...
typedef enum {
	...
	AUDIO_PCM_RATE_11K      = 11025,        /**< 11.025 kHz sample rate*/
	AUDIO_PCM_RATE_16K	= 16000,	/**< 16 kHz sample rate */
	AUDIO_PCM_RATE_22K      = 22050,        /**< 22.05 kHz sample rate*/
...
struct audio_codec_api {
	...
	int (*route_input)(const struct device *dev, 
			audio_channel_t channel,
			uint32_t input);
	int (*route_output)(const struct device *dev, 
			audio_channel_t channel,
			uint32_t output);
};
/**
 ...
 * @return 0 if successful, negative errno code if failure.
 */
static inline int audio_codec_route_input(const struct device *dev,
					audio_channel_t channel,
					uint32_t input)
{
	const struct audio_codec_api *api =
 ...
 * @return 0 if successful, negative errno code if failure.
 */
static inline int audio_codec_route_output(const struct device *dev,
					audio_channel_t channel,
					uint32_t output)
{
	const struct audio_codec_api *api =
